...
int main(void) {
    ...
    while( 1 ){
    	...
    	if( notify ){
    		...
    		uart_send_string("AT+CMGD=1,4\r");	//clear the text messages
    		notify=0;
    	}
		else{
			processGPS();
...
void configureGSM(){
	...
	uart_send_string( "AT+CMGD=1,4\r"); //delete all messages
...
void configureGPS(){
	...
	uart_send_string("AT#GPSPREFIX=\"&&\"\r");
	uart_send_string("AT#GPSURAIS=\"ID1111\"\r");
	
	//disable everything except for GPGLL messages
	//contains the longitude, latitude, date which we need
	uart_send_string("AT#GPSGPRMC=0\r");
	uart_send_string("AT#GPSGPGGA=0\r");
	uart_send_string("AT#GPSGPGLL=1\r");
	uart_send_string("AT#GPSGPGSA=0\r");
	uart_send_string("AT#GPSGPGSV=0\r");
	uart_send_string("AT#GPSGPVTG=0\r");
	
	//disable auto gps message sending for now
	uart_send_string("AT#GPSINTERVALLOC=0\r");
...
int parseGPS( const char * gpgllMessage, char * latude, char * lotude ){
	...
	if(sscanf(gpgllMessage, "%*[^$]$%99[^*]*%x%*s", gpsMessage, &sum) == 2){
		if(checksum(gpsMessage, sum)){
			if(sscanf( gpsMessage, "GPGLL,%18[^,],%1s,%18[^,],%1s%*s", latude,
				lat_dir, lotude, long_dir) == 4 ){
...
void processGPS(){
	...
	while(1){
		uart_send_string("AT#GPSGETMESSAGE\r");
		if(parseGPS(gsmBuf, current_longitude, current_latitude)){
			break;
		}
	}
